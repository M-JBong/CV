...
const data = {
  ...
  skills: {
    "Technical":["Organic & organometallic synthesis","Glovebox & Schlenk techniques","Air-sensitive compound handling","Photocatalysis"],
    "Instrumental":["Operando FTIR","Cyclic Voltammetry","Single-crystal XRD","NMR","GC-TCD, HPLC"],
  ...
  teaching: [
    ...
    { type:"TA", activity:"General Chemistry Laboratory 2", period:"Fall 2022", desc:"Supervised lab sessions for general chemistry." },
  ...
  gallery:[
    { title:"Tether Length Study", tag:"Ongoing", desc:"SCL modulation in Mn(I) bpy complexes", hue:"210" },
    { title:"Chem. Sci. 2026", tag:"Published", desc:"Proton relay & CO₂-to-formate conversion", hue:"150" },
    { title:"CURT Award", tag:"2021", desc:"Excellent Research Report Award", hue:"45" },
  ],
};
...
export default function App() {
  ...
  const hdrBg= dark ? "linear-gradient(160deg,#0a0f1e 0%,#111827 50%,#0f1e3a 100%)" : "linear-gradient(160deg,#eef4ff 0%,#ffffff 45%,#e8f0fe 100%)";
  ...
  const SecTitle = ({ icon, children }: { icon?: React.ReactNode; children: React.ReactNode }) => (
  ...
  const Pill = ({ v }: { v: string }) => <span style={{ fontSize:11, padding:"3px 10px", borderRadius:20, border:`1px solid ${bord}`, background:bgS, color:textS, whiteSpace:"nowrap" }}>{v}</span>;
  const Badge = ({ v, green }: { v: string; green?: boolean }) => <span style={{ fontSize:10, padding:"2px 9px", borderRadius:20, fontWeight:600, background:green?(dark?"#052e16":"#f0fdf4"):accLC, color:green?(dark?"#4ade80":"#15803d"):accC, border:`1px solid ${green?(dark?"#166534":"#bbf7d0"):accBC}` }}>{v}</span>;
              ...
              <div key={i}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                  <div style={{ display:"flex", gap:9, alignItems:"flex-start" }}>
                    ...
                    <div>
                      <p style={{ fontSize:13, fontWeight:600, color:text, margin:"0 0 2px" }}>{t.activity}</p>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))", gap:14 }}>
            {data.gallery.map((g, i) => (
              <div key={i} style={{ background:bgC, border:`1px solid ${bord}`, borderRadius:14, overflow:"hidden", boxShadow:dark?"0 2px 12px #00000030":"0 2px 12px #1d4ed808" }}>
                <div style={{ height:96, background:`linear-gradient(135deg,hsl(${g.hue},70%,${dark?"20%":"75%"}) 0%,hsl(${g.hue},55%,${dark?"30%":"90%"}) 100%)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, opacity:0.85 }}>⬡</div>
